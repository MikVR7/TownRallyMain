...
namespace TownRally
{
    internal class RallyCreator
    {
        ...
        private void CreateRallyGrazMur()
        {
            ...
            this.rallyDescription.Add(new Description { Type = DescriptionType.Image, Data = "descr_00.jpg" });
            this.rallyDescription.Add(new Description { Type = DescriptionType.Text, Data = "In der Stadt und auf der Höh’ – das schließt sich in Graz keineswegs aus. Der Fluss, die einzigartige Altstadt und mitten drin ein Berg. Der Grazer Schlossberg ist Sehenswürdigkeit, Naturschauspiel, Naherholungsgebiet und Aussichtspunkt zugleich. In kürzester Zeit gelangt man nach oben und genießt den herrlichen Ausblick auf Graz, die Altstadt und Umgebung. Einer Burg, die vor über 1.000 Jahren auf dem Schlossberg errichtet wurde, verdankt die Stadt ihren Namen. Aus dem slawischen Gradec für ,kleine Burg‘ wurde später Graz.\r\n\r\nSeit 1894 überwindet die Schlossbergbahn bravourös die rund 60% Steigung hinauf zum Schlossbergplateau - mit modernen Panoramagondeln.\r\n\r\nEine schnellere Art den Schlossberg zu erreichen, ist die Fahrt mit dem Schlossberglift." });
            string rallyID = CreateRally("grazmur1", "MurRally", "Entdecke die Mur in Graz", this.rallyDescription.ToArray());
            ...
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hop hop, auf zum Mur!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hop hop, auf zum Mur! Zum zweiten Mal!" } });

            // station 2
            stationID = this.CreateStation(rallyID, 41.223f, 15.321f, new int[0]);
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hop hop, auf zum Mur, Station 2!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hop hop, auf zum Mur, Station 2! Zum zweiten Mal!" } });

            DatabaseHandler.EventIn_SaveRallyWhole.Invoke(rallyID, rally,
                new Dictionary<string, Station>(this.stations), new Dictionary<string, Task>(this.tasks));
        }
        ...
        private void CreateRallyGrazSchlossberg()
        {
            ...
            string rallyID = CreateRally("grazschlossb1", "Schloßberg Rally", "Ausblick inmitten der Altstadt", this.rallyDescription.ToArray());
            ...
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Begib dich zum Kameliterplatz (Karmeliterpl., 8010 Graz)" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hallo und herzlich willkommen bei der<br><b><size=130%>Schloßberg-Townrallye!" } });
            this.CreateTask(stationID, Task.Type.Game_Objectfind, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Um die Rallye zu starten, suche mit deinem Handy eine Teufelsfigur am Kamerliterplatz!" } });
        ...
        private void CreateRallyOedWald()
        {
            ...
            string rallyID = CreateRally("zauberwald00", "Zauberwald", "Willkommen im Zauberwald", this.rallyDescription.ToArray());
            ...
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Begib dich zum ersten Teil des Waldstücks!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Willkommen zum ersten Waldstück!" } });

            // station 2
            stationID = this.CreateStation(rallyID, 47.061213f, 15.866783f, new int[1] { 3 });
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Gehe durch den Wald und finde den Zweiten Wegpunkt!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Willkommen beim zweiten Waldstück!" } });
            this.CreateTask(stationID, Task.Type.Game_Objectfind, new Description[1] { new Description { Type = DescriptionType.Image, Data = "game00.jpg" } });

            // station 3
            stationID = this.CreateStation(rallyID, 47.060489f, 15.868046f, new int[1] { 4 });
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Ab zum nächsten wegpunkt! Jetzt geht es direkt ins Dickicht." } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Hast du dich verirrt? Hier wohnt die Waldhexe!" } });

            // station 4
            stationID = this.CreateStation(rallyID, 47.057089f, 15.871939f, new int[2] { 5, 6 }, 7);
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Tiefer in den Wald! Los los los!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Auf dieser Weggabelung musst du dich entscheiden! Welchen Wegpunkt willst du zuerst aufsuchen?" } });

            // station 5
            stationID = this.CreateStation(rallyID, 47.060899f, 15.871932f, new int[0]);
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Suche den tiefsten, tiefsten Wald auf!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Die Geister tanzen hier!" } });

            // station 6
            stationID = this.CreateStation(rallyID, 47.051357f, 15.871713f, new int[0]);
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Finde die Waldlichtung!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Die Waldlichtung erleuchtet dir den Weg..." } });

            // station 7
            stationID = this.CreateStation(rallyID, 47.052431f, 15.875369f, new int[0]);
            // tasks
            this.taskCreationCounter = 0;
            this.CreateTask(stationID, Task.Type.GotoDestination, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Raus aus dem Wald!" } });
            this.CreateTask(stationID, Task.Type.InfoScreen, new Description[1] { new Description { Type = DescriptionType.Text, Data = "Herzlichen Glückwunsch! Du hast die Oeder Waldrunde gut überstanden." } });

            DatabaseHandler.EventIn_SaveRallyWhole.Invoke(rallyID, rally,
                new Dictionary<string, Station>(this.stations), new Dictionary<string, Task>(this.tasks));
        }

        private string CreateRally(string rallyNameInternal, string rallyName, string caption, Description[] descr)
        {
        ...
        private void CreateTask(string stationID, Task.Type taskType, Description[] descr)
        {
            Task task = new Task();
